...
import com.eshipz.qa.utils.ExcelData2;
import com.eshipz.qa.utils.ExcelData3;
...
public class ForwardShipmentTest extends BaseClass{
	...
	String excel_upload_file= pro.getProperty("uploadfile");
	String excel_write_file= pro.getProperty("writedata");
	Utilities utils;
	ForwardCreateShipmentPage create_shipment_pg;
	ShipmentsPage shipment_pg;

	public ForwardShipmentTest() {
		super();
	}
	...
	public void createShipmentThruExcel()throws InterruptedException, EncryptedDocumentException, IOException, AWTException
	{
		...
		create_Ship_page = excel_pg.selectUpload(excel_upload_file);
		...
		//create_Ship_page.closeNotificationPresent();
		int number_f_orders = create_Ship_page.eachOrder.size(); 
		System.out.println("---------------ORDERS----------"+number_f_orders);  
		//ele_data.writeToFile(number_f_orders); 

//		ArrayList<String> data_frm_bulkexlfile = ExcelData.readingFromExcel("bulk_upload", excel_upload_file);
		ArrayList<String> data_frm_bulkexlfile = ExcelData3.readingFromExcel("bulk_upload", excel_upload_file);
		utils.removeNullnEmpty(data_frm_bulkexlfile);
		List<String> expected = utils.clipoffdecimal(data_frm_bulkexlfile);
		System.out.println("expected"+expected);		
		
//		ArrayList<String> data_frm_createshipmentpg = ExcelData.newreadingFromExcel("Sheet2", excel_write_file);
		ArrayList<String> data_frm_createshipmentpg = ExcelData3.newreadingFromExcel("Sheet2", excel_write_file);
		//ArrayList<String> data_frm_createshipmentpg = ExcelData.readingFromExcel("Sheet2", "C:\\Users\\Windows\\eclipse-workspace\\EshipzProject\\ObjectRepo\\src\\main\\java\\com\\eshipz\\qa\\testdata\\data_to_xl.xlsx");
		List<String> actual = utils.removeNullnEmpty(data_frm_createshipmentpg);
		System.out.println("after removing spaces  --> "+actual);
